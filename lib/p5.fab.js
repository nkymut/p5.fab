...
class Fab {
  ...
  parseGcode() {
    this.vertices = [];
    let lastPos = new p5.Vector(0, 0, 0); // Track last known position
    let isRelativeMode = false; // Track positioning mode
    ...
    _fab.commands.forEach((cmd) => {
      ...
      var newV = new p5.Vector(lastPos.x, lastPos.y, lastPos.z);
      let vertexData = {
        command: code,
        vertex: newV,
        full: fullcommand,
      ...
      cmd.forEach((c) => {
        let val = parseFloat(c.substring(1));
        switch (c.charAt(0)) {
          case "X":
            if (isRelativeMode) {
              newV.x = lastPos.x + val;
            } else {
              newV.x = val;
            }
            hasCoordinates = true;
          ...
          case "Y":
            if (isRelativeMode) {
              newV.z = lastPos.z + val; // switch z-y
            } else {
          ...
          case "Z":
            if (isRelativeMode) {
              newV.y = lastPos.y + -1 * val; // switch z-y
            } else {
      ...
      if (newV && hasCoordinates) {
        this.vertices.push(vertexData);
        lastPos.set(newV); // Update last known position
      }
    });
  }
  ...
  render() {
    ...
    if (this.vertices.length > 0) {
      ...
      if (!this.model) {
        this.model = this.createToolpathModel();
        //model(this.model);
      } else {
        //model(this.model);
        //p5.Geometry not working
        this.drawToolpath();
      }
    }
  ...
  createToolpathModel() {
    push();
    // Initialize toolpath position and geometry
    let toolpathPos = new p5.Vector(0, 0, 0);
    let lastCommand = "G0";

    // Create geometry for toolpath
    let toolpathGeom = new p5.Geometry();
    toolpathGeom.gid = "toolpath";
    toolpathGeom.vertices = [];
    toolpathGeom.vertexColors = [];

    for (let v = 0; v < this.vertices.length; v++) {
      const vertexData = this.vertices[v];
      if (vertexData.command === "G0") {
        if (lastCommand === "G0") {
          // G0->G0: Travel move to travel move (no extrusion)
          toolpathGeom.vertices.push(toolpathPos, toolpathPos, vertexData.vertex);
          toolpathGeom.vertexColors.push([0, 1, 0, 0.4], [0, 1, 0, 0.4], [0, 1, 0, 0.4]); // Green for travel
        } else {
          // G1->G0: Extrusion to travel move (retraction)
          toolpathGeom.vertices.push(toolpathPos, toolpathPos, vertexData.vertex);
          toolpathGeom.vertexColors.push([1, 0, 0, 0.4], [1, 0, 0, 0.4], [1, 0, 0, 0.4]); // Red for retraction
        }
        toolpathPos.set(vertexData.vertex.x, vertexData.vertex.y, vertexData.vertex.z);
      } else if (vertexData.command === "G1") {
        if (lastCommand === "G0") {
          // G0->G1: Travel move to extrusion (priming)
          toolpathGeom.vertices.push(toolpathPos, toolpathPos, vertexData.vertex);
          toolpathGeom.vertexColors.push([0, 0, 1, 0.4], [0, 0, 1, 0.4], [0, 0, 1, 0.4]); // Blue for priming
        } else {
          // G1->G1: Extrusion to extrusion (printing)
          toolpathGeom.vertices.push(toolpathPos, toolpathPos, vertexData.vertex);
          toolpathGeom.vertexColors.push([0, 0, 1, 0.4], [0, 0, 1, 0.4], [0, 0, 1, 0.4]); // Blue for printing
        }
        toolpathPos.set(vertexData.vertex.x, vertexData.vertex.y, vertexData.vertex.z);
      }
    ...
    // Set geometry mode to TRIANGLES
    toolpathGeom.mode = TRIANGLES;

    // Create and return model from geometry
    return toolpathGeom;
  }
  ...
  drawToolpath() {
    ...
    let prevPos = new p5.Vector(0, 0, 0);
    let prevCommand = "G0";

    for (let v = 0; v < this.vertices.length; v++) {
      const vertexData = this.vertices[v];
      let currentPos = new p5.Vector(0, 0, 0);

      // console.log("vertexData",vertexData);
      // console.log("prevPos",prevPos);
      // console.log("currentPos",currentPos);

      vertexData.full.includes("X") ? (currentPos.x = vertexData.vertex.x) : (currentPos.x = prevPos.x);
      vertexData.full.includes("Z") ? (currentPos.y = vertexData.vertex.y) : (currentPos.y = prevPos.y);
      vertexData.full.includes("Y") ? (currentPos.z = vertexData.vertex.z) : (currentPos.z = prevPos.z);

      // Skip first vertex since we need two points to draw a line
      if (v > 0) {
        //push();
        if (vertexData.command === "G0") {
          if (prevCommand === "G0") {
            // G0->G0: Travel move to travel move (no extrusion)
            stroke(0, 255, 0, 100); // Green for pure travel
          } else {
            // G1->G0: Extrusion to travel move (retraction)
            stroke(255, 0, 0, 100); // Red for retraction
          }
        } else if (vertexData.command === "G1") {
          // Both G0->G1 (priming) and G1->G1 (printing) in blue
          stroke(0, 0, 255, 100); // Blue for printing/priming
        }

        // Draw line from previous to current position using beginShape/vertex
        beginShape(LINES);
      ...
      // Update previous position and command for next iteration
      //console.log("currentPos",currentPos);
      prevPos.set(currentPos.x, currentPos.y, currentPos.z);
      prevCommand = vertexData.command;
    }
  }
  ...
  makeE(x, y, z) {
    return (dist(this.asyncPosition.x, this.asyncPosition.y, x, y) * (this.nozzleR / this.filamentR) ** 2).toFixed(2);
  }
